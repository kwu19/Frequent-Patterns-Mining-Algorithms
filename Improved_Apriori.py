...
def partition_data(data_set, num_par):
    """
    Seperate the dataset into different number of partitions without repeat
    Input:
        data_set: a list of tuples contains attributes in each tuple.  
        num_par: number of partitions that want to seperate the data into
    Output:
        result: the data after partitioning
    """
    avg = len(data_set) / float(num_par)
    result = []
    tmp = 0.0

    while tmp< len(data_set):
        result.append(data_set[int(tmp):int(tmp + avg)])
        tmp += avg
    return result
...
def generate_Lk(data_set, Ck, min_sup, sup_set):
    ...
    for item in _count:
        if (_count[item] / t_num) >= min_sup:
            Lk.add(item)
            sup_set[item] = _count[item] / t_num
    return Lk
...
def improved_apriori(data_set, num_par, min_sup):
    """
    This method is to implement the improved apriori algorithm to the dataset that we read.   
    Input: 
        data_set: a list of tuples contains attributes in each tuple.
        num_par: number of partitions that user want
    ...
    """
    par_data = partition_data(data_set, num_par)
    k = len(data_set[0])
    
    for i in range(0, num_par+1):
        L, sup_set = generate_L(par_data[0], k, min_sup)  # generate frequent items according to each partition
        association_rules = generate_association(L, sup_set, min_conf = 0.7)  #  generate association rules according to each partition
...
def main():
    ...
    improved_apriori(data_set, num_par, min_support)
